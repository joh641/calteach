...
Then /the reservation under "(.*?)" should be for (.*?) days/ do |user_email, days|
  ...
  days = days.to_i
  if res.reservation_out != nil
    assert(days.business_days.after(res.reservation_out.to_datetime+8.hours).to_date == res.reservation_in)
  else
    assert(days.business_days.after(res.date_out.to_datetime+8.hours).to_date == res.reservation_in)
  end
end
