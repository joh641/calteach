...
describe ReservationsController do
  ...
  describe 'changing due date of a reservation' do
    it 'should save properly' do
      ...
      reservation.save
      put :update, {:id => reservation.id, :reservation => {:reservation_in => (Date.today + 3).strftime("%m/%d/%Y")}}
  ...
  describe 'changing due date of a reservation causing conflict' do
    it 'should error out' do
      ...
      reservation1.save
      reservation2.save
      expect { put :update, {:id => reservation1.id, :reservation => {:reservation_in => (Date.today + 3).strftime("%m/%d/%Y")}} }.to raise_error 
  ...
  describe 'changing due date of a reservation that was overdue' do
    it 'should save properly' do
      ...
      reservation1.save
      reservation2.save
      put :update, {:id => reservation1.id, :reservation => {:reservation_in => (Date.today + 1).strftime("%m/%d/%Y")}}
      assert Reservation.find(reservation1.id).reservation_in == Date.today + 1
      reservation1.destroy
      reservation2.destroy
    end
  ...
  describe 'inputting invalid date' do
    it 'should error out' do
      reservation = Reservation.new({:user_id => 1, :item_id => 1, :quantity => 1, :date_out => Date.today, :reservation_in => Date.today + 1, :notes => nil})
      reservation.save
      expect { put :update, {:id => reservation.id, :reservation => {:reservation_in => "asdf"}} }.to raise_error 
